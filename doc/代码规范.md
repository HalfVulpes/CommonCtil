...
所有的**定义**都必须写在源文件里面，例如全局变量的**定义**和函数的**实现**等。
...
所有的**声明**都必须写在头文件里面，例如宏定义、函数的**声明**（`extern cmtXxxx();`）、全局变量的**声明**（`extern cmtUint64 xxx;`）以及结构体的**声明**等。
...
```c
extern cmtUint8 cmtAaaaBbbb(cmtUint64 param);
```
...
```c
extern cmtUint8 cmtU8toU16(cmtU8str* u8, cmtU16str* u16);
extern cmtUint8 cmtReadJsonObj(cmtU8str* str);
```
...
* @author dexnab
*/
extern cmtUint8 cmtU82U16(cmtU8str* utf8, cmtU16str* utf16);
```
...
| 状态码 | 全称          | 解释                                                         |
| ------ | ------------- | ------------------------------------------------------------ |
| NC     | Not Completed | 未完成                                                       |
| NT     | Not Tested    | 未测试                                                       |
| PA     | Passed        | 测试通过                                                     |
| NU     | Need Upgrade  | 需要升级（当前版本可以通过测试，没有影响正常使用的bug，但需要提升性能或修改功能） |
| NF     | Need Fix      | 需要修复（当前版本无法通过测试，有影响性能的bug）            |
| OD     | Outdated      | 已过时（应添加注释`@see NewFunc()`指向新函数）               |
...
命名规则可以遵循**宏定义**的命名规则，也可以遵循**普通函数**的命名规则。
